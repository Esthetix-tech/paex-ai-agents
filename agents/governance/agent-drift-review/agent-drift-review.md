...
title: Agent Drift Review
description: Draft-only and review-only governance intake specification for identifying agent drift risks and preparing human review materials without granting registry, routing, activation or production authority.
layer: governance
...
exemption_scope:
  ...
  - no_registry_record_modification
  - no_direct_quarantine_action
...
forbidden_actions:
  ...
  - directly_modify_drift_result_or_registry_record
  - directly_quarantine_agent_without_review
allowed_actions:
  - review_metadata
...
For agent drift review specifically, this file must not directly modify drift results, alter registry records or directly quarantine an agent. It may recommend HOLD, ESCALATE, remediation, human review or Guardian Review only.
